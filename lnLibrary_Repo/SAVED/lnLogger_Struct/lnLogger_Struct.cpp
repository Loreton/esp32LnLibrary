...
const char* ESP32LoggerMutex::getTimeStamp(uint32_t millisec, bool fStrip) {
    ...
    if (millisec == 0) {
        ...
        millisec = esp_timer_get_time() / 1000; // Tempo in microsecondi dall'avvio
        // millisec = rtc.getMillis(); // Tempo in millisecondi
        // millisec = this_rtc.getMicros() / 1000; // Tempo in millisecondi
    }
...
const char* ESP32LoggerMutex::getFileLineInfo(const char* file, int line) {
    static char buffer[25];                                         // spazio per il [filename.linNo]
...
void ESP32LoggerMutex::write(const char* color, const char* tag, const char* file, int line, const char* format, ...) {
    ...
    if (m_logMutex != NULL && xSemaphoreTake(m_logMutex, portMAX_DELAY) == pdTRUE) {
        ...
        Serial.printf("%s[%s][%s][%s] %s%s\n",
                      ...
                      this->getTimeStamp(0),         // get elapsed time from boot
