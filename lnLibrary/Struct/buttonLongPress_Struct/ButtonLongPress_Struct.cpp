...
#include "lnTime.h" // mSecTo_HHMMSS()
...
void ButtonLongPress_Struct::_checkNewLevel() {
    ...
    if (m_elapsed - lastDisplayTime > 60000) {
        ...
        LOG_NOTIFY("\telapsed:            %s", mSecTo_HHMMSSms(m_elapsed));
        LOG_NOTIFY("\ttime to next level: %s", mSecTo_HHMMSSms(ms_to_next_level));
