...
void ButtonLongPress_Struct::_checkNewLevel() {
    ...
    LOG_TRACE("[%s]currentPressTime: %lu, m_currentPressLevel %d, new_level %d", m_pinID, currentPressTime, newLevel);

    for (int8_t i = m_numThresholds - 1; i >= 0; i--) {
        LOG_TRACE("[%s]m_pressThresholds[i]: %02d) %lu", m_pinID, i, m_pressThresholds[i]);
...
void showStatus(ButtonLongPress_Struct *p) {
    ...
    LOG_DEBUG("\t%-18s: %2d",        "m_currentPressLevel",        p->m_currentPressLevel);
    LOG_DEBUG("\t%-18s: %2d",        "m_lastPressedLevel",         p->m_lastPressedLevel);
