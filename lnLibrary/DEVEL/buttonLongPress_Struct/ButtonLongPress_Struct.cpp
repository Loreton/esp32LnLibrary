...
void ButtonLongPress_Struct::init(const char* name, uint8_t pin,
    ...
    for (int8_t i = 0; i<m_numThresholds-1; i++) {
        ...
        // LOG_DEBUG("    threshold[%d]: %6lu - GAP: %6lu", i, m_pressThresholds[i], m_gapThresholds[i]);
    }
...
void ButtonLongPress_Struct::_checkNewLevel() {
    ...
    ext_showStatus(this);
    LOG_DEBUG("[%s] currentPressTime: %lu, m_currentPressLevel %d, new_level %d", m_pinID, currentPressTime, m_currentPressLevel, newLevel);
...
/** con bouncing ma non funziona bene
...
bool ButtonLongPress_Struct::read_(ButtonCallback onPressCallback) {
...
//#
//###########################################################################
/*void ButtonLongPress_Struct::showStatus2(ButtonCallback showStatus_callback) {
