...
// bool isTimerExpired(structMillisTimer *t) {
...
// structMillisTimer asyncTIMER, timer15MIN;
// structMillisTimer *asyncTimer = &asyncTIMER;
//##############################################################
//# ref:
//#      https://github.com/espressif/arduino-esp32/blob/master/libraries/WiFi/examples/WiFiScanAsync/WiFiScanAsync.ino
//##############################################################
// void wifi_asyncScan() {
//     if (asyncTimer->start == 0) {
//         startMillisTimer(asyncTimer, 3*60*1000);
//     } else {
//         if (isTimerExpired(asyncTimer)) {
//             printf0_NFN("async_Scan starting\n");
//             WiFi.scanNetworks(true);  // 'true' turns Async Mode ON
...
bool processScannedNetworks(int16_t networksFound) {
    // bestssid->available = false;
    bestssid->is_better = false;
    int8_t RSSI_LEVEL = 0; // -10
    printf0_FN("WiFi Scan found %d networks\n", networksFound);
    for (int i = 0; i < networksFound; ++i) {

        // displayNetwork(i, networksFound);

        int8_t ssid_index = isSSIDinList(WiFi.SSID(i).c_str(), &mySSID[0]);
    ...
    if (!WiFi.isConnected()) {
        // flag for new best SSID available
        // bestssid->available = true;
        bestssid->is_better = true;
    }
    else if (bestssid->diff_rssi <= RSSI_LEVEL) { // new RSSI better than 10 points than current
        printf0_FN("new SSID %s (RSSI %d) is better than current one:", bestssid->ssid, bestssid->rssi);
        printf0_FN("%s (RSSI %d) (NO reconnection necessary)\n", WiFi.SSID().c_str(),  WiFi.RSSI());
        // bestssid->available = true; // flag for new best SSID available
        bestssid->is_better = true; // flag for new best SSID available
    } else {
        printf0_FN("new SSID %s (RSSI %d) is not enough better than current one:", bestssid->ssid, bestssid->rssi);
        printf0_FN("%s (RSSI %d) (NO reconnection necessary)\n", WiFi.SSID().c_str(),  WiFi.RSSI());
    }

    return bestssid->available;
}
